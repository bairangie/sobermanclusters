...
if p.MakeSvg
    try
        print(figHan,[fileName '.svg'],p.SvgOptions{:},resStr)
    catch err
        warning('MFFIGUREEXPORT:expFailure',['Error exporting to svg:' err.message])
    end
end
if nargout > 0
    varargout{1} = success;
